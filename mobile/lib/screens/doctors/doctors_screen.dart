      ...
      body: Column(children: [
        ...
        Expanded(child: _loading ? const AppLoading() : filtered.isEmpty ? const Center(child: Text('No doctors found')) : ListView.builder(
          ...
          itemBuilder: (_, i) => Card(margin: const EdgeInsets.only(bottom: 12), child: ListTile(
            ...
            leading: CircleAvatar(radius: 30, backgroundColor: const Color(0xFF2563EB).withValues(alpha: 0.1), backgroundImage: ApiConfig.buildImageUrl(filtered[i].avatar) != null ? CachedNetworkImageProvider(ApiConfig.buildImageUrl(filtered[i].avatar!)!, headers: ApiConfig.imageHeaders) : null, child: ApiConfig.buildImageUrl(filtered[i].avatar) == null ? Text(filtered[i].name.isNotEmpty ? filtered[i].name[0] : 'D', style: const TextStyle(color: Color(0xFF2563EB), fontSize: 20, fontWeight: FontWeight.bold)) : null),
